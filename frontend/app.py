...
# Adicionar Produto
with st.expander("Agregar un Nuevo Producto"):
    with st.form("new_product"):
        name = st.text_input("Nombre del Producto")
        description = st.text_area("Descriçpcion del Producto")
        price = st.number_input("Precio", min_value=0.01, format="%f")
        categoria = st.selectbox(
            "Categoria",
            ["Eletrônico", "Eletrodoméstico", "Móveis", "Roupas", "Calçados"],
        )
        email_fornecedor = st.text_input("Correo Electronico")
        submit_button = st.form_submit_button("Ingresar Produto")
